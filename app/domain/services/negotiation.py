...
class NegotiationService:
    ...
    async def start_negotiation(self, asset_id: int, buyer_id: str, role: str) -> Tuple[bool, str, Optional[AssetNegotiation]]:
        ...
        stmt = select(AssetNegotiation).where(
            AssetNegotiation.asset_id == asset_id, 
            AssetNegotiation.buyer_id == int(buyer_id), 
            AssetNegotiation.status == NegotiationStatus.OPEN
        )
        result = await self.db.execute(stmt)
        if result.scalar_one_or_none(): 
            return False, "لديك بالفعل جلسة تفاوض مفتوحة ونشطة لهذا الأصل.", None

        new_negotiation = AssetNegotiation(
            asset_id=asset_id, 
    ...
    async def close_negotiation(self, negotiation_id: int, close_status: str) -> Tuple[bool, str]:
        ...
        if not negotiation: 
            return False, "جلسة التفاوض المستهدفة غير موجودة."
        if negotiation.status != NegotiationStatus.OPEN: 
            return False, f"فشل الإغلاق: الجلسة ليست مفتوحة حالياً ({negotiation.status})."
    ...
    async def create_offer(self, negotiation_id: int, amount: float, actor_id: str, role: str) -> Tuple[bool, str]:
        from sqlalchemy import select
        # استخدام joinedload لمنع Lazy Loading للأصل المرتبط بالجلسة
        stmt = select(AssetNegotiation).options(joinedload(AssetNegotiation.asset)).where(AssetNegotiation.id == negotiation_id)
        res = await self.db.execute(stmt)
        negotiation = res.scalar_one_or_none()
        if not negotiation or negotiation.status != NegotiationStatus.OPEN: 
            return False, "فشل تقديم العرض: جلسة التفاوض مغلقة أو غير موجودة."
        
        allowed, msg = await SecurityGuard.authorize_action(
            db_session=self.db, 
            action="CREATE_OFFER", 
            actor_id=actor_id, 
            actor_name=f"Buyer_{actor_id}", 
            role=role, 
        ...
        event = AssetEvent(
            ...
            payload=f"تقديم عرض مالي جديد بقيمة {amount}$ من المشتري {actor_id}", 
            actor=f"Buyer_{actor_id}", 
            actor_id=actor_id
        )
        self.db.add(event)
        await self.db.flush()
    ...
    async def accept_offer(self, negotiation_id: int, asset_state_service, actor_id: str, role: str) -> Tuple[bool, str]:
        from sqlalchemy import select
        # استخدام joinedload لمنع Lazy Loading للأصل المرتبط بالجلسة لمنع خطأ MissingGreenlet
        stmt = select(AssetNegotiation).options(joinedload(AssetNegotiation.asset)).where(AssetNegotiation.id == negotiation_id)
        res = await self.db.execute(stmt)
        negotiation = res.scalar_one_or_none()
        if not negotiation or negotiation.status != NegotiationStatus.OPEN: 
            return False, "جلسة التفاوض غير موجودة أو مغلقة مسبقاً."
        
        allowed, msg = await SecurityGuard.authorize_action(
            db_session=self.db, 
            action="RESPOND_OFFER", 
            actor_id=actor_id, 
            actor_name=f"Seller_{actor_id}", 
            role=role, 
            negotiation=negotiation
        )
        if not allowed: 
        ...
        success, state_msg = await asset_state_service.transition_status(
            asset_id=negotiation.asset_id, 
            to_state="RESERVED", 
            actor=f"Seller_{actor_id}", 
            actor_id=actor_id, 
            role=role, 
            reason=f"تم قبول العرض المالي في التفاوض رقم {negotiation_id}"
        )
        if not success: 
            return False, f"فشل حجز الأصل: {state_msg}"

        negotiation.status = NegotiationStatus.ACCEPTED
        negotiation.updated_at = datetime.utcnow()

        stmt_others = select(AssetNegotiation).where(
            AssetNegotiation.asset_id == negotiation.asset_id, 
            AssetNegotiation.id != negotiation_id, 
            AssetNegotiation.status == NegotiationStatus.OPEN
        )
        ...
        for other_neg in res_others.scalars().all():
            ...
            self.db.add(AssetEvent(
                asset_id=negotiation.asset_id, 
                event_type="NEGOTIATION_AUTO_CLOSED", 
                payload=f"إغلاق تلقائي للتفاوض رقم {other_neg.id} بسبب قبول عرض منافس.", 
    ...
    async def send_message(self, negotiation_id: int, sender_id: str, role: str, text: str) -> Tuple[bool, str]:
        from sqlalchemy import select
        stmt = select(AssetNegotiation).options(joinedload(AssetNegotiation.asset)).where(AssetNegotiation.id == negotiation_id)
        res = await self.db.execute(stmt)
        negotiation = res.scalar_one_or_none()
    ...
    async def list_messages(self, negotiation_id: int, user_id: str, role: str, limit: int = 10, offset: int = 0) -> Tuple[bool, str, List[Dict[str, Any]]]:
        from sqlalchemy import select
        stmt = select(AssetNegotiation).options(joinedload(AssetNegotiation.asset)).where(AssetNegotiation.id == negotiation_id)
        res = await self.db.execute(stmt)
        negotiation = res.scalar_one_or_none()
